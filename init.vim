...
"honizontal partition window
nmap <Leader>wc <Esc>:sp<CR>
...
"set *.f filetype=text
autocmd BufRead,BufNewFile *.f set filetype=text
"set *.file filetype=csh
autocmd BufRead,BufNewFile *.file set filetype=csh
...
let g:python3_host_prog = '/pub/tools/others/Python-3.7.1.1/bin/python3'
"about coc
...
"indentLine
"dont conceal in json/md file
autocmd BufRead,BufNewFile,BufEnter *.json,*.md let g:indentLine_conceallevel = 0
"when leave, set indentLine_conceallevel default
autocmd BufLeave *.json,*.md let g:indentLine_conceallevel = 2
...
autocmd BufRead,BufNewFile *.md UltiSnipsAddFiletypes md.tex
...
"set cocmenusel color, must behind color scheme set
hi CocMenuSel ctermbg=248 guibg=#bdae93

"enable syntax
syntax enable
"if read md file, then use markdown.vim(~/.config/nvim/colors/markdown.vim), not use default colors
autocmd BufRead,BufNewFile *.md colorscheme markdown

"GitHub Copilot
"The default mapping for accepting a suggestion is <C-l>,
"I change it to <C-x>.
lua << EOF
require("CopilotChat").setup({
  ...
  mappings = {
    reset = {
      normal  = '<C-x>',   -- Clear shortcut in normal mode, default was '<C-l>'
      insert  = '<C-x>',   -- Clear shortcut in insert mode, default was '<C-l>'
    },
  },
  prompts = {
    Explain = {
      prompt = 'Write an explanation for the selected code as paragraphs of text. Please answer in Chinese.',
    },
  },
})
...
require('minuet').setup {
    ...
    virtualtext = {
        auto_trigger_ft = {'c', 'cpp', 'verilog', 'systemverilog', 'python', 'tcsh', 'bash', 'v', 'copilot-chat'},
        keymap = {
